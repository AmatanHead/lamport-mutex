...
from lamport_mutex.utils.log_checker import Checker
...
def make_parser():
    ...
    stress.add_argument('--check', action='store_true',
                        help='run log checker after stresstest')
    ...
    log_check = subparsers.add_parser(
        'log_check', help='check logs generated by stresstest '
                          'and print results',
    )

    log_check.add_argument('--log-dir', type=directory_type, required=True,
                           help='a directory with logs')

    log_check.add_argument('--log', type=argparse.FileType('w'),
                           default=sys.stdout,
                           help='where to write results '
                                '(default is stdout)')

    log_check.add_argument('--lock-file')

    log_check.add_argument('--no-color', action='store_true',
...
def stress_worker(namespace):
    ...
    if os.path.exists(namespace.lock_file):
        os.remove(namespace.lock_file)
